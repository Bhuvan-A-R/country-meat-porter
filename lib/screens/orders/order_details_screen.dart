                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('LIVE GPS TURN-BY-TURN NAVIGATION', style: TextStyle(fontSize: 10, color: Color(0xFF64748B), fontWeight: FontWeight.w900, letterSpacing: 0.8)),
                            Text(
                              order.status == OrderDeliveryStatus.assigned || order.status == OrderDeliveryStatus.arrivedAtStore
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      OutlinedButton.icon(
                        ...
                        onPressed: () {
                          final destination = (order.status == OrderDeliveryStatus.assigned || order.status == OrderDeliveryStatus.arrivedAtStore)
                              ? order.storeAddress
                              : order.deliveryAddress;
                          _launchMaps(context, destination);
                        },
                        icon: const Icon(Icons.near_me_rounded, size: 16, color: Color(0xFF2563EB)),
                        label: const Text('Launch Google Maps', style: TextStyle(color: Color(0xFF2563EB), fontSize: 12, fontWeight: FontWeight.bold)),
                      ),
                    ],
                  ),
                ],
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('CASH ON DELIVERY (COD) MANDATE', style: TextStyle(color: Color(0xFFD97706), fontSize: 10, fontWeight: FontWeight.w900)),
                          Text(
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  const Row(
                    children: [
                      Icon(Icons.storefront_rounded, color: Color(0xFFEA580C), size: 22),
                      SizedBox(width: 10),
                      Text('1. STORE PICKUP HUB', style: TextStyle(fontSize: 10, color: Color(0xFFEA580C), fontWeight: FontWeight.w900, letterSpacing: 0.8)),
                    ],
                  ),
                  const SizedBox(height: 8),
                  Text(order.storeName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Color(0xFF0F172A))),
                  Text(order.storeAddress, style: const TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                  const SizedBox(height: 14),
                  Row(
                    children: [
                      Expanded(
                        child: OutlinedButton.icon(
                          ...
                          onPressed: () => _makePhoneCall(context, order.storePhone),
                          icon: const Icon(Icons.phone_rounded, color: Color(0xFFEA580C), size: 18),
                          label: Text('CALL STORE (${order.storePhone})', style: const TextStyle(color: Color(0xFFEA580C), fontWeight: FontWeight.bold, fontSize: 12)),
                        ),
                      ),
                      const SizedBox(width: 10),
                      IconButton.filled(
                        style: IconButton.styleFrom(backgroundColor: const Color(0xFFEA580C)),
                        icon: const Icon(Icons.near_me_rounded, color: Colors.white, size: 20),
                        onPressed: () => _launchMaps(context, order.storeAddress),
                      ),
                    ],
                  ),
                ],
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 10),
                      Text('2. CUSTOMER DELIVERY LOCATION', style: TextStyle(fontSize: 10, color: primary, fontWeight: FontWeight.w900, letterSpacing: 0.8)),
                    ],
                  ),
                  const SizedBox(height: 8),
                  Text(order.customerName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Color(0xFF0F172A))),
                  Text(order.deliveryAddress, style: const TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'Note: ${order.customerInstructions}',
                    ...
                    children: [
                      Expanded(
                        child: ElevatedButton.icon(
                          ...
                          label: Text('CALL CUSTOMER (${order.customerPhone})', style: const TextStyle(fontSize: 12)),
                        ),
                      ),
                      const SizedBox(width: 10),
                      IconButton.filled(
                        ...
                        icon: const Icon(Icons.near_me_rounded, color: Colors.white, size: 20),
                        onPressed: () => _launchMaps(context, order.deliveryAddress),
                      ),
                    ],
                  ),
                ],
              ...
              child: Column(
                ...
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('Fresh Meat Bag Verification', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Color(0xFF0F172A))),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                        decoration: BoxDecoration(
                          color: verifiedCount == order.items.length ? const Color(0xFFECFDF5) : const Color(0xFFEFF6FF),
                          borderRadius: BorderRadius.circular(6),
                        ),
                        child: Text(
                          '$verifiedCount / ${order.items.length} VERIFIED',
                          style: TextStyle(
                            color: verifiedCount == order.items.length ? const Color(0xFF059669) : const Color(0xFF2563EB),
                            fontSize: 10,
                  ...
                  const Text('Verify each meat & gel-pack seal before leaving hub:', style: TextStyle(fontSize: 12, color: Color(0xFF64748B))),
                  const SizedBox(height: 10),
                  ...List.generate(order.items.length, (idx) {
                    ...
                    return CheckboxListTile(
                      ...
                      title: Text('${item.quantity}x ${item.name} (${item.weight})', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF0F172A))),
                      subtitle: Text('Price: ₹${item.price.toStringAsFixed(0)}', style: const TextStyle(fontSize: 11, color: Color(0xFF64748B))),
                      onChanged: (_) {
          ...
          child: const Row(
            ...
            children: [
              ...
              Text(
                ...
                style: TextStyle(color: Color(0xFF059669), fontWeight: FontWeight.w900, fontSize: 15),
              ),
            ],
          ),
        );
